...
import type { ExpensesCollectionResponse } from "@myakiba/contracts/expenses/schema";
...
export function CollectionSummary({
  ...
  const rows = [
    { label: "Order-linked", value: data?.summary.orderLinked },
    { label: "Standalone", value: data?.summary.standalone },
  ];
    ...
    <ExpensePanel title="Summary" className="h-full lg:h-[400px]">
      <div className="flex h-full flex-col gap-8" aria-busy={isLoading}>
        <div className="flex flex-col gap-1">
          <p className="text-xs text-muted-foreground">Collection Spend</p>
          {isLoading ? (
